...
public static class IntegrationsEndpoints
{
    ...
    public static IEndpointRouteBuilder MapIntegrationsEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        app.MapPost("/integrations/ibkr/reconcile-oms", async (
            TradePro.Api.Oms.IBKRFillReconciler reconciler, CancellationToken ct) =>
        {
            // Body lives in IBKRFillReconciler so the scheduled pass and this
            // manual one can never diverge. Extracted 27 Aug 2026 — until then
            // this logic had NO scheduled caller, so it only ever ran when a
            // human poked it.
            var r = await reconciler.RunAsync(ct);
            if (!r.Ok)
                return Results.Json(new { error = r.Error },
                    statusCode: r.Error == "IBKR disabled" ? 503 : 502);
            return Results.Ok(r.Payload);
        })
