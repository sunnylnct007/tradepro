/**
 * IchimokuFx — dedicated page for the ichimoku_fx_mr strategy.
 *
 * Shows (all on one screen, collapsible CockpitCards):
 *   1. Signal Grid    — latest session decisions
 *   2. Ichimoku Charts — Plotly cloud charts per pair
 *   3. Positions       — T212 demo FX positions
...
const STRATEGY_ID = "ichimoku_fx_mr";
...
function StatePill({ state }: { state: string }) {
  ...
  return (
    <Pill label={state} color={color} bg={color + "22"} />
  );
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 1200 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700 }}>Ichimoku FX</h1>
          <p style={{ margin: "4px 0 0", fontSize: 12, color: C.muted }}>
            ichimoku_fx_mr · G10 pairs
          </p>
        </div>
                ...
                <tbody>
                  {strategyOrders.map((o, i) => (
                    <tr key={o.id} style={i % 2 === 0 ? rowEven : rowOdd}>
                      ...
                      <td style={{ ...tdStyle, textAlign: "right", fontFamily: "monospace" }}>
                        {o.avgFillPrice != null ? o.avgFillPrice.toFixed(4) : "—"}
                      </td>
        ...
        {strategySessions.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {strategySessions.slice(0, 10).map((s) => {
              ...
              const signals = rs?.decisions?.filter(
