...
**🧭 NORTH-STAR VISION (user, 2026-07-02): TradePro = a CENTRAL ORCHESTRATION SYSTEM across multiple brokers — NOT a "unicorn."** The brokers (IBKR/IG/T212) provide execution, data, charts, native positions/P&L (golden source = "the hands"). TradePro is "the brain" on top: **decide** (signals/scanner/COMPASS) → **route** (per-strategy broker, already wired) → **aggregate** (cross-broker/cross-strategy state no single broker shows) → **risk** (sizing/stops/gates) → **journal** (score the human's calls). **Don't reinvent the wheel:** for operational DETAIL (per-position, fills, charts, per-account P&L) leverage/deep-link the broker's own UI; TradePro owns only the CROSS-cutting intelligence + honesty layer (e.g. the P&L-truth card). This is why it was "fit for nothing" before — it chased the unicorn instead of orchestrating. [[project_tradepro_as_filter_reuse_ibkr]]
...
**PARKED — IBKR intraday-granularity harvest (for the intraday phase):** we harvest **yfinance (bronze)**, which caps intraday at ~7-30 days of 1-min and **no sub-minute** (why the 1m cache is only ~43 days for a subset). **Daily (1d) = full 66 months (2021→now), cross-checked accurate** — plenty for the daily Ichimoku focus (uses 1d only). **IBKR gives far more granularity** (months of 1-min + 30-sec/5-sec/tick, gold via MCP `get_price_history` steps or the gateway). **Parked because** it's not needed until an INTRADAY strategy. **When intraday is the focus:** opportunistic IBKR harvest that runs when the gateway is IDLE (overnight/weekends — the hang was contention, not IBKR) + per-request timeout/skip + dedicated data gateway (clientId 132 already wired). This is how we finally tap the IBKR pro-account granularity.
...
- ✅ **fill-replay layers 2-3 (2026-06-29):** C# `fill_replay_results` table (migration 051) + `FillReplayEndpoints` (read `/api/fill-replay/{strategy}/latest` + ingest `/api/ingest/fill-replay`, mirror equity-pipeline) + `FillReplayCard.tsx` on the cockpit + `api.fillReplay()` client method. Builds clean both sides. **DEPLOYED + SEEDED + VERIFIED end-to-end** (push ok:200, read endpoint returns data, card renders). **Scheduled:** `com.tradepro.fill-replay-push.plist` → `scripts/fill-replay-push.sh` runs `tradepro-fill-replay --push` 21:45 BST Mon-Fri (after the 21:30 harvest); verified via launchd kickstart. **✅ data-quality fix (2026-06-29):** large_50 cache coverage was only 14/50 (36 names missing — clone traded them via yfinance-live fallback, not the managed cache). Backfilled all 50 deep (2021→now, ~1376 bars each) → **50/50 now ≥200 bars**, and since they're in the cache dir the daily harvest auto-keeps them fresh (no wrapper change). Fill-replay large_50 jumped n=8→22 (skipped 15→1); the read is now real: **corr(ext,ret) +0.37** (more-extended large_50 entries did BETTER — momentum healthy, reinforces "no extension cap on large_50"). Card fixed to present corr neutrally + gate the staggered counterfactual to one-shot baskets only (no false signal on organic entries).

**QUEUED FEATURES (priority order, 2026-06-29):**
1. **✅ BUILT (2026-06-29): "Today's Setups" scanner (pillar #2 at dashboard level).** `tradepro-today-setups` CLI scans a universe → per-symbol Ichimoku signal + range/risk + kijun → ranks by ENTRY QUALITY (⭐ consider = LONG + ≤2 ATR above kijun + <90th pctile; ⚠ extended = LONG but 90th+/>3 ATR; excluded = not LONG). Vertical: migration 052 `today_setups_results` + `TodaySetupsEndpoints` (ingest /api/ingest/today-setups + read /api/today-setups/{universe}/latest) + `TodaySetupsCard` on the dashboard (after FillReplayCard) + `api.todaySetups`. Built own CLI (not tradepro-compare — that's watchlist-enum-bound + runs slow backtests) reusing the proven artifact-push pattern + my range/risk compute. **Fixes the old flat-BUY-light board:** curated + ranked + risk context + honest ⭐/⚠ (never a confident BUY). First run on large_50: 8 ⭐ (CSCO/GS/MS/JPM at kijun support) · 14 ⚠ · 28 excluded · 50/50 covered. Daily --push scheduled (`com.tradepro.today-setups-push` 21:50 BST Mon-Fri, after harvest+fill-replay; verified via kickstart). Also scans high_beta (card merges both, tagged L50/HB + ATR).
   **🐛 BUG FIX (2026-06-30, user/agent-caught — 3 trust-breakers):** (1) **negative ATR-above-kijun** (price BELOW support) was classified ⭐ and *ranked best* (sort asc → most-negative first) — GSL/HON false positives. Now `consider` requires **0≤dist_atr≤2** (at/above kijun); below-kijun = `weak`, not shown. (2) **garbage bars** (HON 18.9%/day ATR, null price) produced fabricated ⭐ → data-quality guard (ATR>25%/day = `suspect`, discarded). (3) scanner used the clone's 5/32/50 Ichimoku but the chart + `get_market_state` use **standard 9/26/52** → "above cloud" disagreed (GSL above fast cloud / below standard) → scanner + `SymbolRangeRiskCard` now use **9/26/52 to match the chart** the user cross-checks. Re-seeded both universes; ⭐ all clean positives, weak/suspect counts surfaced on the card. **ROUND-2 FIX (2026-06-30, agent caught again — divergence from get_market_state):** patching my PARALLEL Ichimoku kept surfacing new divergences (PG 'inside cloud', JPM 'WAIT') because the root cause was the parallel computation itself. **Strategic fix: the scanner is now DRIVEN BY `market_state()` — the same engine get_market_state uses** — for cloud position + BUY/WAIT/AVOID verdict + the isolated-spike garbage guard (so it CANNOT contradict get_market_state). On top, added the **parabolic/near-high HARD VETO the engine itself lacks** (range≥90, or range≥80 & 3m-mom>60, or mom>100 → extended): a pullback to kijun after +150% ≠ after +15%. Verified vs the agent's table — JPM→excluded(WAIT), COHR/AMKR/STX/WDC→extended(parabolic), AEIS/ANET/PG/MS-zone→consider. The scanner only RANKS by kijun-proximity on top of the canonical verdict now. **Lesson: the kijun-distance number alone is not a signal — cloud-above + at/above-kijun + clean-data are all required.** **NEXT:** add earnings-proximity (catalyst gap); kijun into market_state for the SCREENERS view.
2. **🔨 IN PROGRESS: Manual trade booking = PILLAR #3 (decision→outcome journal).** ✅ Phase 1 backend BUILT (2026-06-30): migration 053 `manual_trades` + `ManualTradesEndpoints` (POST/GET/PATCH-close, P&L computed long/short-aware on close). NEXT phases: (2) MCP tools; (3) nightly P&L + stop/target alert; (4) UI tab. — Solves the session-conflict (engine auto-fires → 2nd session duplicates) AND the real gap: nowhere to record a trade from YOUR OWN analysis vs the engine's signals. Spec (user's agent, sound): `manual_trades` table (symbol, side, qty, entry_price, entry_time, stop_loss, take_profit, reason, source, checklist_score, status, exit_price/time/reason, pnl_usd/pct) — SEPARATE from the engine's order flow (zero conflict, independent P&L track). Endpoints: POST /api/paper/manual-trade, GET /api/paper/manual-trades, PATCH /api/paper/manual-trade/{id}/close. MCP tools: book_manual_trade / close_manual_trade / get_manual_trades / get_manual_trade_journal. Nightly job: pull live price per open trade → unrealised P&L + alert on stop/target hit. UI: manual-trades tab (low priority). This is the journal that scores the user's OWN calls over time ([[project_tradepro_as_filter_reuse_ibkr]] pillar #3).

**PILLAR #1+#2 surfacing started (2026-06-29) — `SymbolRangeRiskCard`** on the per-symbol rail (next to the systematic signal): 52w-range position, %off-high, vs-200SMA, ATR(14), the **kijun pullback level** (lower-risk discretionary entry) + the **8% stop level** (size-to-stop). Frames the JEF/KO/MO "buying near highs" debate correctly: the systematic signal buys strength (NOT vetoed — extended large_50 did fine per fill-replay), and this card gives the human the discretionary-timing + risk-sizing context on top ("systematic signal, discretionary entry, systematic risk"). Computed client-side from `api.candles` (no fabricated data; says so when history is thin).

**OPERATIONAL DECISION (user, 2026-06-29) — keep ALL paper strategies RUNNING, do NOT disable the shelved ones.** Even though the focus is ONE strategy, the FX/intraday daemons stay live IN PAPER because that's free history + behavioural data feeding the measurement loop (fill-replay/attribution) — matches [[feedback_start_simple_observe]]. NB FX (`ichimoku_fx_mr_ibkr`) places a ~9-pair one-shot basket daily (e.g. 04:50 UTC) — the SAME one-shot-basket pattern we flagged in equity; worth a fill-replay on FX later. **Go-live safety is a per-strategy REAL-MONEY gate, NOT a shutdown:** only the focus strategy routes to a real account; everyone else stays paper. Make real-money enablement explicit + auditable in `strategy_runtime_config` / broker map at go-live ([[project_going_live_real_money]], [[project_config_driven_daemon]]) — don't re-suggest stopping the paper daemons.
...
- **Data**: live wheel needs OPRA (US options) — paywalled. **USD 32.75/mo**, verified against IBKR Account Management 29 Aug 2026 ("OPRA (US Options Exchanges) (P,L1)"). CORRECTED from an earlier note here claiming ~$125/mo, which was wrong by ~4x and materially overstated the decision: $393/yr, not $1,500/yr. The (P,L1) tag confirms the account IS classified Professional, so there is no non-pro discount — but the professional OPRA price is far lower than the old note implied. Separately, NYSE American/BATS/ARCA/IEX (Network B) is $25/mo and is what SPY *stock* top-of-book needs.
  Owner's call 29 Aug: **do not buy yet** — evaluate both markets on free data first (Yahoo EOD chains, already captured nightly by option-chain-capture), and revisit once a month of paper record exists. OPRA is US-only and does nothing for the Indian strategy, which is the one actually being traded.
