...
- ✅ **fill-replay layers 2-3 (2026-06-29):** C# `fill_replay_results` table (migration 051) + `FillReplayEndpoints` (read `/api/fill-replay/{strategy}/latest` + ingest `/api/ingest/fill-replay`, mirror equity-pipeline) + `FillReplayCard.tsx` on the cockpit + `api.fillReplay()` client method. Builds clean both sides. **NEXT:** schedule the daemon `--push` (so the card auto-refreshes daily) + first manual `tradepro-fill-replay --push` to seed it. Also surfaced: large_50 cache coverage is partial (fill-replay skipped 15 `_US_EQ` names) → harvest should track the strategy universe.
